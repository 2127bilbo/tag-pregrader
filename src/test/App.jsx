...
// 
function scanEdgeBands(d,w,h,bn){
  const {left:cl,right:cr,top:ct,bottom:cb,cardW:cW,cardH:cH}=bn;
  const L=(y,x)=>LUM(d[(y*w+x)*4],d[(y*w+x)*4+1],d[(y*w+x)*4+2]);
  const LINES=9;
  const scanSide=(side)=>{
    const outerHits=[], innerHits=[];
    for(let li=0;li<LINES;li++){
      const f=0.15+0.70*(li/(LINES-1));
      let from, to, len, sample;
      if(side==='L' || side==='R'){
        const y=Math.round(ct+cH*f);
        from = side==='L' ? cl : cr;
        to   = side==='L' ? cr : cl;
        len = Math.abs(to-from);
        sample = (i)=>{
          const x = side==='L' ? (from+i) : (from-i);
          const xm=CLAMP(x-1,0,w-1), xp=CLAMP(x+1,0,w-1);
          const y0=CLAMP(y,0,h-1);
          return Math.abs(L(y0,xp)-L(y0,xm));
        };
      }else{
        const x=Math.round(cl+cW*f);
        from = side==='T' ? ct : cb;
        to   = side==='T' ? cb : ct;
        len = Math.abs(to-from);
        sample = (i)=>{
          const y = side==='T' ? (from+i) : (from-i);
          const ym=CLAMP(y-1,0,h-1), yp=CLAMP(y+1,0,h-1);
          const x0=CLAMP(x,0,w-1);
          return Math.abs(L(yp,x0)-L(ym,x0));
        };
      }
      const maxDepth=Math.min(len, Math.round(Math.min(cW,cH)*0.35));
      const grads=[];
      let gSum=0,gN=0;
      ...
      const gStd=gN?Math.sqrt(grads.reduce((s,v)=>s+((v||0)-gMean)**2,0)/gN):0;
      const gThresh=Math.max(2.0, gMean + gStd*2.0);
      let outer=null, inner=null;
      for(let i=2;i<maxDepth-2;i++){
        if(grads[i]>gThresh){ outer=i; break; }
      }
      if(outer!==null){
        for(let i=outer+4;i<maxDepth-2;i++){
          if(grads[i]>gThresh){ inner=i; break; }
        }
      }
      if(outer!==null) outerHits.push(outer);
      if(inner!==null) innerHits.push(inner);
    }
    const med = arr => { const s=[...arr].sort((a,b)=>a-b); return s.length?s[Math.floor(s.length/2)]:null; };
    return {outer:med(outerHits), inner:med(innerHits)};
  };

  const Ls=scanSide('L'), Rs=scanSide('R'), Ts=scanSide('T'), Bs=scanSide('B');
  return {L:Ls,R:Rs,T:Ts,B:Bs};
}
...
// ─── Full pipeline ────────────────────────────────────────────────────────────
async function analyzeCard(src, manualRotate=0) {
  ...
  const angle=angleResult.angle + (manualRotate||0);
  // Stage 3: deskew
  ...
  const bands=scanEdgeBands(dd.data,dw,dh,bounds);
  const bL=bands.L.inner||bands.L.outer||0;
  const bR=bands.R.inner||bands.R.outer||0;
  const bT=bands.T.inner||bands.T.outer||0;
  const bB=bands.B.inner||bands.B.outer||0;
  const lrT=bL+bR, tbT=bT+bB;
  const lrRatio=lrT>0?Math.round((bL/lrT)*1000)/10:50;
  const tbRatio=tbT>0?Math.round((bT/tbT)*1000)/10:50;
  ...
  const scanStub = (w,mode)=>({width:w,confidence:mode==='fallback'?'failed':'good',debug:{mode}});
  const centeringFull={
    bL,bR,bT,bB,lrRatio,tbRatio,
    colorL:cL,colorR:cR,colorT:cT,colorB:cB,
    scanL:scanStub(bL,bands.L.inner?'mode1':'mode2'),
    scanR:scanStub(bR,bands.R.inner?'mode1':'mode2'),
    scanT:scanStub(bT,bands.T.inner?'mode1':'mode2'),
    scanB:scanStub(bB,bands.B.inner?'mode1':'mode2'),
    confidence:'good',
    clamped:false
  };
  const displayUrl=deskewed.toDataURL('image/jpeg',0.92);
  return {displayUrl,dw,dh,bounds,centering:centeringFull,angle,angleResult,deskewApplied,bands};
}
...
// ─── Card Panel ───────────────────────────────────────────────────────────────
function CardPanel({label,side,onResult,manualRot,onManualRot}){
  ...
  const handleFile=e=>{
    ...
    reader.onload=async ev=>{
      ...
      try{
        const res=await analyzeCard(src, manualRot||0);
        setResult(res);
    ...
    <div style={{flex:1,minWidth:0,display:'flex',flexDirection:'column',gap:10}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
        ...
        <div style={{display:'flex',gap:6}}>
          {result&&<div style={{display:'flex',alignItems:'center',gap:4,marginRight:6}}>
            <span style={{fontFamily:mono,fontSize:7,color:'#444'}}>ROT</span>
            <button onClick={()=>onManualRot&&onManualRot((manualRot||0)-0.2)} style={{padding:'2px 6px',borderRadius:4,border:'1px solid #333',background:'transparent',color:'#666',fontFamily:mono,fontSize:8,cursor:'pointer'}}>-</button>
            <span style={{fontFamily:mono,fontSize:8,color:'#888',minWidth:38,textAlign:'center'}}>{(manualRot||0).toFixed(1)}°</span>
            <button onClick={()=>onManualRot&&onManualRot((manualRot||0)+0.2)} style={{padding:'2px 6px',borderRadius:4,border:'1px solid #333',background:'transparent',color:'#666',fontFamily:mono,fontSize:8,cursor:'pointer'}}>+</button>
          </div>}
  ...
  return(
    <div style={{minHeight:'100vh',background:'#090b0e',color:'#ccc',fontFamily:sans}}>
      ...
      <div style={{padding:14,display:'flex',gap:12}}>
        <CardPanel label="Front" side="front" onResult={setFR} manualRot={manualRot} onManualRot={setManualRot}/>
        <CardPanel label="Back"  side="back"  onResult={setBR} manualRot={manualRot} onManualRot={setManualRot}/>
      </div>
      {fc&&bc&&(
